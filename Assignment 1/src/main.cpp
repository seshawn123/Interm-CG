...
int main() {
	...
	{ 
		...
		shader->SetUniform("cell_shading",cellShading); 
		...
		//Texture2D::sptr snowSpec = Texture2D::LoadFromFile("images/snow_Specular.jpg");
		...
		GameObject penguin = scene->CreateEntity("penguin");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/A#1 Models/penguin.obj");
			penguin.emplace<RendererComponent>().SetMesh(vao).SetMaterial(stoneMat);
			penguin.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
			penguin.get<Transform>().SetLocalRotation(0.0f, 0.0f, 0.0f);

			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(penguin);

			//Binded to pathing
			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(penguin);
			//Setting up the path
		...
		EnvironmentGenerator::AddObjectToGeneration("models/simplePine.obj", simpleFloraMat, 20,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/A#1 Models/penguin.obj", simpleFloraMat, 50,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		...
		EnvironmentGenerator::AddObjectToGeneration("models/A#1 Models/igloo.obj", snowMat, 10,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		...
		std::vector<KeyPressWatcher> keyToggles;
		{
			...
			controllables.push_back(penguin);
